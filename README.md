...
**그래서 용량이 아니라 채널 수를 사야 한다.** 같은 8채널 보드를 채우는 네 가지 방법의
대역폭과 토큰 생성 속도는 **전부 같다** — 다른 것은 용량과 가격뿐이다.

| 구성 | RAM | 실효 대역폭 | decode | RAM 비용(추정) |
|---|---|---|---|---|
| 8×8GB | 64 GB | 221.2 GB/s | 46.8 tok/s | $440 |
| 8×16GB | 128 GB | 221.2 GB/s | 46.8 tok/s | $760 |
| 8×32GB | 256 GB | 221.2 GB/s | 46.8 tok/s | $1,200 |
| 8×64GB | 512 GB | 221.2 GB/s | 46.8 tok/s | $2,320 |

7B 모델이 실제로 쓰는 것은 7.4GB다. **8×8GB로 채우면 성능은 동일하고 RAM 비용은 3분의 1**이다.
용량은 모델이 들어가기만 하면 되고, 나머지 예산은 채널이 더 많은 CPU에 쓰는 편이 낫다.
